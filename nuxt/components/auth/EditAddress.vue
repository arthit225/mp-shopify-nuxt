<template>
<SnippetsBackgroundBlocker :zIndex="20" @click="$emit('toggle-edit-form')">
  <div id="add-new-address" 
    class="fixed
        ...
        @click.stop
  >
    <button @click="$emit('toggle-edit-form')" class="absolute top-4 right-4">
      <IconsCloseX />
    </button>
    <h1 class="text-extra-large text-center mb-4">Edit Address</h1>
    ...
    >
      ...
      <div class="flex flex-col form-field">
        ...
        <select
          ...
          :data-default="formData && formData.node.countryCodeV2 ? formData.node.countryCodeV2 : 'US'"
      ...
      <div class="flex flex-col form-field">
        ...
        <select
          ...
          :data-default="formData && formData.node.provinceCode ? formData.node.provinceCode : 'CA'"
...
const emit = defineEmits(['edit-address-completed', 'toggle-edit-form']);
...
const state = reactive({
  first_name: formData.node.firstName ? formData.node.firstName : '',
  last_name: formData.node.lastName ? formData.node.lastName : '',
  company: formData.node.company ? formData.node.company : '',
  address1: formData.node.address1 ? formData.node.address1 : '',
  address2: formData.node.address2 ? formData.node.address2 : '',
  city: formData.node.city ? formData.node.city : '',
  countryCodeV2: formData.node.countryCodeV2 ? formData.node.countryCodeV2 : 'US',
  provinceCode: formData.node.provinceCode ? formData.node.provinceCode : '',
  zip: formData.node.zip ? formData.node.zip : '',
  phone: formData.node.phone ? formData.node.phone : '',
});
...
async function updateAddress() {
  ...
  try {
    const updatedAddressData = await useShopify({ query, variables });
    customerAddress = updatedAddressData.data.customerAddress;
    customerUserErrors = updatedAddressData.data.customerUserErrors;
  } catch (err) {
    console.log(err);
